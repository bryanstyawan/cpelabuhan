...
$(document).ready(function(){
	...
	$("#btn-trigger-controll").click(function(){
		...
		else
		{
			if (flag_allowed == 1) {
				$.ajax({
					url :"<?php echo site_url();?>master/vessel/store",
					type:"post",
...
function edit(id){
	$.ajax({
		url :"<?php echo site_url();?>master/vessel/get_data/"+id+"/ajax/mr_vessel",
		type:"post",
		beforeSend:function(){
			$("#loadprosess").modal('show');
		},
...
{					
	Lobibox.confirm({
		...
		callback: function ($this, type) {
			if (type === 'yes'){			
				$.ajax({
					url :"<?php echo site_url();?>master/vessel/store/"+'delete/'+id,
					type:"post",
					beforeSend:function(){
						$("#editData").modal('hide');
						$("#loadprosess").modal('show');
